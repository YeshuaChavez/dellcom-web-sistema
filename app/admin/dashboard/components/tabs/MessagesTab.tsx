  ...
  return (
    <section className="animate-fade-in-up">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {[
          { label: "Sin Leer", value: filteredMensajes.filter(m => !m.leido).length, icon: "mark_email_unread", bg: "bg-red-50 text-red-600" },
          ...
          <div key={i} className="group bg-white border border-slate-200 p-5 rounded-2xl shadow-sm flex items-center gap-4 hover:shadow-md hover:-translate-y-0.5 transition-all duration-300">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5"><span className="font-extrabold text-slate-800 text-base">{item.value}</span> mensajes</p>
            </div>
          </div>
        ))}
      </div>
